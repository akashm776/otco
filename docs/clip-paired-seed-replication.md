...
**Next check, prepared but not run:** [measure five checkpoints across the same three seed trajectories](clip-paired-intermediate.md) to see how the early benefit weakens. Add updates 250, 500 and 750, and replay endpoints 100 and 1,001 as reproducibility gates. Do not choose a universal switch-off step from these two endpoints. Learning rates, optimizer moments and relative auxiliary pressure change with stage, so this does not isolate representation geometry.
