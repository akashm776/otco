...
def run(source, output, protocol=None, *, evaluation_observer=None):
    ...
    if evaluation_observer is not None:
        evaluation_observer.initialize(processor, data)
    ...
    for step in steps:
        ...
        if evaluation_observer is not None:
            evaluation_observer.start_checkpoint(model, step)
        ...
        for trial, batch in enumerate(training_batches):
            ...
            for arm in protocol['arms']:
                ...
                transfer_losses = (evaluation_observer.evaluate(model)
                                   if evaluation_observer is not None else None)
                if arm == 'baseline':
                    ...
                    if trial == 0:
                        ...
                        if evaluation_observer is not None and evaluation_observer.evaluate(model) != transfer_losses:
                            raise AssertionError('Repeated native transfer evaluation differs')
                ...
                all_rows.append(row)
                if evaluation_observer is not None:
                    evaluation_observer.record(row, transfer_losses)
    ...
    if evaluation_observer is not None:
        evaluation_observer.finish(provenance)
