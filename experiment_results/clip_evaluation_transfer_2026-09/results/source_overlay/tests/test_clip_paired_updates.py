...
def test_transfer_observer_does_not_change_actual_adamw_branches(tmp_path):
    from src.clip_evaluation_transfer import TransferObserver
    model, optimizer, scheduler, batch, config, initial = fixture()
    observer = TransferObserver(tmp_path/'transfer', 789)
    observer.batches = [batch]
    observer.conditions = {'sequential': {'batches': [list(range(10))]}}
    for arm, coefficient in [('baseline',0.), ('uniform_top8',.06), ('hardest_real',.375)]:
        restore_branch(model, optimizer, scheduler, initial, 9)
        expected_metrics, expected_delta = one_update(model, optimizer, batch, config,
            arm=arm, coefficient=coefficient, seed=9)
        scheduler.step()
        expected_state = state_digest(dict(model=model.state_dict(), optimizer=optimizer.state_dict(),
                                          scheduler=scheduler.state_dict()))
        restore_branch(model, optimizer, scheduler, initial, 9)
        observer.start_checkpoint(model, 100)
        metrics, delta = one_update(model, optimizer, batch, config, arm=arm, coefficient=coefficient, seed=9)
        scheduler.step()
        observer.evaluate(model)
        assert metrics == expected_metrics
        assert torch.equal(delta, expected_delta)
        assert state_digest(dict(model=model.state_dict(), optimizer=optimizer.state_dict(),
                                 scheduler=scheduler.state_dict())) == expected_state
