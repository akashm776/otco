...
CONFIG_NAME = "hf_flickr30k_softmax_mix_k32"
...
checkpoint_dir = f"/content/drive/MyDrive/otco_checkpoints/{CONFIG_NAME}"
...
if os.path.exists(latest):
    print(f"Found latest.pt — will resume from: {latest}")
else:
...
subprocess.run(["git", "-C", repo_dir, "add", "experiments/", f"results/"], check=False)
