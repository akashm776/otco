"""One foreground Colab job: three seeds, five states, 720 probes, local-only ZIP."""
...
def verify_completion(output):
    ...
    if result != expected:
        raise AssertionError('All three seeds and five checkpoints must complete')
    from src.clip_paired_intermediate import verify_seed
    study = json.loads((output / 'results/protocol.json').read_text())
    for seed in [42, 123, 456]:
        verify_seed(output / f'seed_{seed}/results/paired', study)
...
def main():
    ...
    run_id = 'clip_paired_intermediate_' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S_%fZ')
    ...
    for package in ['torch', 'torchvision']:
        ...
        if actual != expected:
            raise RuntimeError(f'Exact endpoint replay requires {package}=={expected}; runtime has {actual}. No training started.')
    subprocess.run([sys.executable, '-m', 'pytest', '-q', 'tests/test_clip_paired_intermediate.py', 'tests/test_clip_paired_seed_replication.py',
    ...
    manifest = {'run_id': run_id, 'source_commit': args.source_commit, 'training_seeds': [42, 123, 456], 'checkpoint_steps': [100,250,500,750,1001],
    ...
    command = [sys.executable, '-u', '-m', 'src.clip_paired_intermediate', '--output-directory', str(output)]
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    print('INTERMEDIATE_OUTPUT:', output, '\nSTORAGE: local Colab disk only; no Drive writes.', flush=True)
    failure = None
    try:
