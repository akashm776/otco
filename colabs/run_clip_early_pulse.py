...
def main():
    ...
    run_id = 'clip_early_pulse_' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S_%fZ')
    ...
    subprocess.run([sys.executable, '-m', 'pytest', '-q', 'tests/test_clip_early_pulse.py',
                    'tests/test_clip_warmup_readiness.py', 'tests/test_clip_gradient_stages.py', 'tests/test_clip_training.py',
    ...
    command = [sys.executable, '-u', '-m', 'src.clip_early_pulse',
               '--output-directory', str(output), '--checkpoint-directory', str(checkpoint)]
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    print('EARLY_PULSE_OUTPUT:', output, flush=True)
    try:
